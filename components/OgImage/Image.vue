...
<template>
    <div class="w-full h-full flex flex-col justify-center bg-[#0A0A0A]">
        ...
        <div class=" pl-[100px]">
            <p v-if="headline" class="uppercase text-[24px] text-[#22C55E] mb-4 font-semibold">
                {{ headline }}
            </p>
            <h1 v-if="title" class="m-0 text-[75px] font-semibold mb-4 text-white flex items-center">
                <span>{{ title }}</span>
            </h1>
            <p v-if="description" class="text-[32px] w-[1000px] text-[#E4E4E7] leading-tight">
                {{ description }}
            </p>
        </div>
